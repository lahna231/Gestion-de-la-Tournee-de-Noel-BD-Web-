...
// Recherche des entretiens lorsqu'on soumet le formulaire
if (isset($_POST['search'])) {
    ...
    // Requête SQL pour rechercher dans les colonnes 'id_elfe', 'id_puce' et 'id_traineau'
    $searchQuery = "SELECT * FROM entretenir WHERE LOWER(id_elfe) LIKE :searchTerm OR LOWER(id_puce) LIKE :searchTerm OR LOWER(id_traineau) LIKE :searchTerm";
    ...
    // Vérifier si le mot-clé "renne" est dans le terme de recherche pour afficher le lien vers renne.php
    ...
    // Si aucune recherche n'est effectuée, récupérer tous les entretiens
    $searchQuery = "SELECT * FROM entretenir";
    $conn = connexion_OCI();
    $stid = oci_parse($conn, $searchQuery);
    oci_execute($stid);
}
...
<div class="container">
    ...
    <form method="POST">
        <input type="text" name="searchTerm" placeholder="Recherche par elfe, renne ou traineau" value="<?php echo $_POST['searchTerm'] ?? ''; ?>" required>
    ...
    <!-- Liens conditionnels pour rediriger vers elfe.php, renne.php ou traineau.php -->
    ...
    <?php if (isset($renneLink)): ?>
        <div>
            <a href="renne.php" style="color:red;">Cliquez ici pour voir les détails des Rennes</a>
        </div>
    <?php endif; ?>
