...
// Recherche des entrées dans la table 'mener' lors de la soumission du formulaire
if (isset($_POST['search'])) {
    ...
    // Requête SQL pour rechercher dans les colonnes 'id_elfe' et 'id_traineau'
    $searchQuery = "SELECT * FROM mener WHERE LOWER(id_elfe) LIKE :searchTerm OR LOWER(id_traineau) LIKE :searchTerm";
    ...
    // Si aucune recherche n'est effectuée, récupérer toutes les entrées de la table 'mener'
    $searchQuery = "SELECT * FROM mener";
    $conn = connexion_OCI();
    $stid = oci_parse($conn, $searchQuery);
    oci_execute($stid);
}
...
<div class="container">
    <h1>Gestion de Table Mener</h1>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <input type="text" name="searchTerm" placeholder="mot-clé ou bien tapez 'elfe' ou bien 'traineau' " value="<?php echo $_POST['searchTerm'] ?? ''; ?>" required>
    ...
    <!-- Liens conditionnels pour rediriger vers elfe.php ou traineau.php -->
