...
// Recherche des entrées dans la table 'lister' lors de la soumission du formulaire
if (isset($_POST['search'])) {
    ...
    // Requête SQL pour rechercher dans les colonnes 'id_enfant' et 'id_jouet'
    $searchQuery = "SELECT * FROM lister WHERE LOWER(id_enfant) LIKE :searchTerm OR LOWER(id_jouet) LIKE :searchTerm";
    ...
    // Vérifier si le mot-clé "enfant" est dans le terme de recherche pour afficher le lien vers enfant.php
    ...
    // Si aucune recherche n'est effectuée, récupérer toutes les entrées de la table 'lister'
    $searchQuery = "SELECT * FROM lister";
    $conn = connexion_OCI();
    $stid = oci_parse($conn, $searchQuery);
    oci_execute($stid);
}
...
<head>
    <meta charset="UTF-8">
    <title>Gestion des Listes d'Enfants et Jouets</title>
    <style>
...
<div class="container">
    <h1>Gestion de La Table Lister</h1>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <input type="text" name="searchTerm" placeholder="mo-clé ou bien tapez  'enfant' ou 'jouet'" value="<?php echo $_POST['searchTerm'] ?? ''; ?>" required>
    ...
    <!-- Liens conditionnels pour rediriger vers enfant.php ou jouet.php -->
    <?php if (isset($enfantLink)): ?>
        <div>
            <a href="enfant.php" style="color:red;">Cliquez ici pour voir les détails des Enfants</a>
        </div>
    <?php endif; ?>
