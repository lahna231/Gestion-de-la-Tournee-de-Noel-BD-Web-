...
// Recherche des envois lorsque le formulaire est soumis
if (isset($_POST['search'])) {
    ...
    // Requête SQL pour rechercher dans les colonnes 'id_traineau', 'id_entrepot' et 'date_envoie'
    $searchQuery = "SELECT * FROM send WHERE LOWER(id_traineau) LIKE :searchTerm OR LOWER(id_entrepot) LIKE :searchTerm OR TO_CHAR(date_envoie, 'YYYY-MM-DD') LIKE :searchTerm";
    ...
    // Vérifier si le mot-clé "traineau" est dans le terme de recherche pour afficher le lien vers traineau.php
    ...
    // Si aucune recherche n'est effectuée, récupérer tous les envois
    $searchQuery = "SELECT * FROM send";
    $conn = connexion_OCI();
    $stid = oci_parse($conn, $searchQuery);
    oci_execute($stid);
}
...
<div class="container">
    <h1>Gestion de la Table Send</h1>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <input type="text" name="searchTerm" placeholder=" mot-clé ou Bien tapez 'entrepot' ou Bien 'traineau'" value="<?php echo $_POST['searchTerm'] ?? ''; ?>" required>
    ...
    <!-- Liens conditionnels pour rediriger vers traineau.php ou entrepot.php -->
    <?php if (isset($traineauLink)): ?>
        <div>
            <a href="traineau.php" style="color:red;">Cliquez ici pour voir les détails des Traineaux</a>
        </div>
    <?php endif; ?>
