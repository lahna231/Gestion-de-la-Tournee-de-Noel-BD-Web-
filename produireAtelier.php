...
// Recherche des entrées dans la table 'produireAtelier' lors de la soumission du formulaire
if (isset($_POST['search'])) {
    ...
    // Requête SQL pour rechercher dans les colonnes 'id_atelier' et 'id_jouet'
    $searchQuery = "SELECT * FROM produireAtelier WHERE LOWER(id_atelier) LIKE :searchTerm OR LOWER(id_jouet) LIKE :searchTerm";
    ...
    // Si aucune recherche n'est effectuée, récupérer toutes les entrées de la table 'produireAtelier'
    $searchQuery = "SELECT * FROM produireAtelier";
    $conn = connexion_OCI();
    $stid = oci_parse($conn, $searchQuery);
    oci_execute($stid);
}
...
<head>
    <meta charset="UTF-8">
    <title>Gestion des Produits Ateliers</title>
    <style>
...
<div class="container">
    <h1>Gestion de la Table produireAtelier</h1>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <input type="text" name="searchTerm" placeholder="mot-clé ou bien tapez 'jouet' ou 'atelier'" value="<?php echo $_POST['searchTerm'] ?? ''; ?>" required>
    ...
    <!-- Liens conditionnels pour rediriger vers jouet.php ou atelier.php -->
